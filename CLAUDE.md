...
- **Trailing-underscore = private (entry points must NOT use it):** Apps Script treats any function whose name ends in `_` as private — it's hidden from the editor's Run/Debug dropdown and can't be a time-driven **trigger handler**. So anything meant to be run from the editor or fired by a trigger (`setupDatabase`, `selfTest`, `generateRecurringTasks`, `installRecurringTrigger`, `doGet`/`doPost`) has **no** trailing underscore; only internal helpers do (`generateForRule_`, `withLock_`). Getting this wrong is silent — `selfTest()` still passes because it calls the internal helper directly, but the trigger never fires and the installer won't appear in the editor (feature 004). When adding a trigger/editor entry point, name it public and, if possible, exercise it (not just its inner helper) in `SelfTest.js`.
- **Trigger install needs `script.scriptapp`:** `ScriptApp.newTrigger()`/`getProjectTriggers()` require the `script.scriptapp` OAuth scope in `appsscript.json`. The manifest lists scopes explicitly, so Apps Script does **not** auto-detect it — add the scope, `clasp push --force` (manifest change), redeploy, and the deploying account re-authorizes once.
...
- **Never commit:** `.clasprc.json` (clasp credentials). The repo is **public** (required for GitHub Pages); Sheet/script IDs are still fine to commit (gated by ID-token + allowlist, not secrecy) but keep them in one config file. No personal data (names, dates, emails, lists, locations) in tracked files — personal data lives only in the Google Sheet.
- **Two users forever.** If a change introduces roles, tenancy, or "scale," it's wrong — simplify.

## Design

All UI work follows `DESIGN.md` (Claude-app-inspired warm palette, dashboard-first landing). Use the Impeccable skill: `/impeccable critique` while iterating, `/impeccable polish` before merging UI work.

### Design Context (from PRODUCT.md)

Register: **product**. Two users (Max, Jaz) only — coordination tool, not a general product. Personality: calm, warm, unfussy — "kitchen corkboard, not Jira board." Dashboard is home (the Home dashboard is the landing view); the calendar is the primary secondary navigation and the organizing metaphor for scheduled items — tasks tether visually to their events inside calendar views. Owner color coding (Max/Jaz/Both) is identity, never decoration. Accessibility target: WCAG 2.1 AA. Full detail: `PRODUCT.md`.
