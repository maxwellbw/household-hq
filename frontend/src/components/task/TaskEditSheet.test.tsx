...
describe('TaskEditSheet', () => {
  ...
  it('saves the new owner when changed, and closes immediately without awaiting', () => {
    mutate.mockClear()
    const onClose = vi.fn()
  ...
  it('sends dueDate: "" when Clear date is used', () => {
    mutate.mockClear()
    render(<TaskEditSheet task={baseTask} onClose={vi.fn()} />)
  ...
  it('seeds the notes field from the task and submits an edited value', () => {
    mutate.mockClear()
