...
## Implementation notes — Polish / Phase 9 (written back per constitution VII)

- **`--accent-strong` token added (T033, beyond the C1 enumeration)**: US1 shipped the terracotta text/fill pairs at the 3:1 UI-component bar and explicitly deferred their small-text restyle to T033 (see the FR-005 note above). The a11y sweep found the real usages that carry *small* text — accent-colored link buttons ("See all", "Open in calendar"), active bottom-nav labels, the selected day-strip tile, the calendar view switcher, the "Both" owner badge, and `DogWalkNotice` — all landing at 3.76–4.05:1. Rather than darken `--accent` itself (which US1 froze in light and which reads correctly as a *fill*), a sibling `--accent-strong` was added: `#a34a2d` light, `#e08a6d` dark, both clearing 4.5:1 on every surface they touch. `--owner-both` now aliases `--accent-strong` (its dominant use is the white-on-fill "MJ" badge, which needs the text-safe value). The contrast script gained an `accent-strong`/`owner-both` pair block so this is now CI-gated, not just hand-checked.
- **Contrast-script parser hardened (T033)**: `scripts/contrast-check.mjs`'s declaration regex matched `--token:` patterns inside comments, so the multi-line prose documenting `--accent-strong` was read as a bogus declaration and crashed the run. Fixed by stripping `/* */` comments before parsing — a latent bug the moment any token grew an explanatory comment.
- **F-20 a11y anomaly resolved, not merely explained (T033)**: axe-core 4.12 (WCAG 2.0/2.1 A+AA) run in-browser on Dashboard, Calendar, Tasks, Lists, More-hub, and all five More leaves, in **both** themes. The tree-read "empty accessible names" on calendar item / day-strip / More-menu buttons was confirmed to be a *tree-reader artifact*: axe and the DOM both compute correct names (`aria-label` on day tiles, visible text content on the rest). Real defects axe *did* surface — and their fixes — are logged in audit.md's Round-3 section. Net: 9 surfaces × 2 themes with **zero** axe violations after fixes.
- **Settings toggle + disabled buttons restyled (T033, FR-023)**: the switch off-state was `bg-surface` knob on `bg-border` track (near-invisible); now the track carries its own border and the knob a contrasting border in both states, so on/off reads at a glance. Disabled primary/enable buttons were `opacity-50` over an accent fill (washed terracotta under white, well under 3:1); now they drop to a flat `surface-alt` + `ink-faint` inert state (~4.3:1, and disabled controls are contrast-exempt regardless) so "nothing to save" reads as a state, not a broken button.
- **Viewport zoom re-enabled (T033, WCAG 1.4.4)**: `index.html` dropped `maximum-scale=1.0, user-scalable=no` (axe `meta-viewport`). Those flags had been suppressing iOS Safari's focus auto-zoom on sub-16px inputs; the cause is now removed at the source instead — `index.css` bumps form controls to 16px under `@media (pointer: coarse)` while desktop keeps the tighter 14px.
- **T034 validation result**: full quickstart §1–§9 exercised against the running build with a `mintDevSessionToken` session (sandbox auth). Theme resolution/switch (§1), full-surface dark sweep (§2), contrast + axe a11y (§5), desktop layout (§6), tasks horizon grouping + persisted shared owner filter (§7), favicon + theme-color adaptation (§8), and regression gates (§9: `npm run build` clean, 575/575 tests pass, `check:contrast` green both themes) all verified live. Dashboard reading-order (§3) and Undo/error-state feedback (§4) were confirmed structurally in-browser and are covered by the unit suite; their live mutation paths (task complete↔reopen, list delete↔re-add) were left to the passing tests rather than writing to the real household ActivityLog during validation, and are flagged for a real-device check before merge (consistent with the US3 note above).
- **T035 result**: `/impeccable audit` on the finished surface scored 20/20 with no unwaived findings — every issue it would have raised was already fixed in T033. No `/impeccable polish` pass was needed.
