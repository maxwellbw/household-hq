...
- [X] T004 In `backend/Config.js` — **deviation**: the seed table is `SETTINGS_SEED` (not `DEFAULT_SETTINGS`); added `pushEnabled=TRUE`, `vapidPublicKey=''`, `vapidPrivateKey=''`, `vapidSubject='mailto:household@example.com'`; added `pushEnabled` to `EDITABLE_SETTINGS`; added ActivityLog action labels `push-notify`/`push-subscribe`/`push-unsubscribe`.
